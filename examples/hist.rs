...
fn histogram(vals: &[(f32, f32)], min: f32, max: f32, bins: usize) -> Vec<(f32, f32)> {
    let mut output = vec!(0; bins + 1);

    let step = (max - min) / (bins + 1) as f32;

    for &(_x, y) in vals.iter() {
        if y < min || y > max {
            continue;
        }
...
fn main() {
    ...
    let hist = histogram(&points, 6.0, 15.0, 16);
    println!("\ny = histogram bars");
    ...
    println!("\ny = some lines");
    Chart::new(180, 60, 0.0, 50.0)
        .lineplot( Shape::Lines(&points) )
        .display();

    println!("\ny = some steps");
    Chart::new(180*2, 60, 0.0, 100.0)
